...
- Registry builds Next.js 16.2.12 production output (run `pnpm --filter @agentpack/registry build` for the current page/route counts), including the `/admin/packs` page and the `/api/admin/packs/[publisher]/[pack]/versions/[version]/status` POST route.
