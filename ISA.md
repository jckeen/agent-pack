...
- [x] ISC-255: Coverage lives in `packages/db/tests/schema.test.ts`, `packages/cli/tests/publish-containment.cli.test.ts`, `packages/cli/tests/publish-variant-containment.cli.test.ts`, `apps/registry/tests/publish-route.test.ts`, `packages/core/tests/policy.test.ts`, and `packages/core/tests/registry-client.test.ts` (the original CLI test names were planned names).
...
- **2026-05-18 (iteration-4) — Worktree strategy:** 5-way dispatch was over-parallelized for a contract-heavy scaffold; type drift on publish/read contracts is the dominant risk. Collapsed to **3 worktrees** + a **protocol commit landed in main first**:
  ...
  - **W1 — Foundation:** full `packages/db` Drizzle schema, queries, migrations, tests + `packages/core/src/policy/{schema,load,enforce}.ts` + `packages/core/src/cache/blobStore.ts`. ISC-151..179, ISC-241..254.
...
- [x] ISC-326 (security): `@agentpack/connector` is now auth-by-default — `AGENTPACK_CONNECTOR_TOKEN` (≥32 chars) required or fail-closed start, constant-time bearer compare, `/healthz` public / `/mcp` authenticated, DNS-rebinding Host/Origin allowlist. 4 → 33 connector tests.
...
- [x] ISC-342: `planUpdate` (`core/install/update.ts`) — per-file reconcile of a fresh `planInstall` against the prior install manifest: LOCAL==BASE → clean update; LOCAL edited + NEW unchanged → retained drift (left alone, reported); both changed → conflict. Marker/JSON files compare the pack's _fragment_ (user content around the span never conflicts, by construction); markerless pack-owned files (skills, agentpack.json) reclassify install-level "no-marker" conflicts via manifest-hash ownership. Removals = BASE outputs absent from NEW (`update-engine.test.ts`, 17 tests).
...
## Sync S3 — user scope + the personal-config loop (2026-07-10, #112)

Phase S3 of `docs/sync-design.md`: `--scope user` installs/updates against `~/.claude`, and `import --into --diff` folds live-config edits back into an existing pack. Workflow doc: `docs/sync.md`.

### Sync-S3 ISCs

- [x] ISC-349: user-scope path mapping — `mapClaudeCodeOutputToUserScope` (claude-code adapter) remaps project layout to the `~/.claude` layout (`.claude/X` → `X`; `CLAUDE.md` stays) and rewrites hook commands `$CLAUDE_PROJECT_DIR/.claude/…` → `$HOME/.claude/…` in `settings.json`. Applied in `planInstall` (new `scope` option, claude-code only) BEFORE the pristine snapshot, so the lockfile, merge fragments, verify, uninstall, and update all agree on mapped paths. `JSON_MERGE_PATHS` gains the mapped `settings.json` form (an existing user settings file deep-merges; user keys survive — asserted e2e).
- [x] ISC-350: user-scope state + provenance — `install --scope user` roots at `~/.claude` (created when missing; mutually exclusive with `--project`; non-claude-code targets are a usage error), state lands at `~/.claude/.agentpack/`, and the install manifest records `scope: "user"` (schema-validated) so `update` re-plans with the same mapping. `update --scope user` targets the `~/.claude` install. `.mcp.json` is an honest ceiling: written under `~/.claude` for reference with a warning — Claude Code reads user-scope MCP servers from `~/.claude.json`, which AgentPack never edits.
- [x] ISC-351: exec gates at user scope — `computeExecDelta` recognizes the user-layout exec surfaces (`hooks/…`, root `settings.json` when the pack ships hooks, root `commands|agents/*.md` bang-bash bodies), keyed off the manifest's recorded scope so project-scope packs can't false-positive; the install-side bang-bash file gate matches the scope's actual layout. A hook-script change on update refuses without `--allow-exec` even with `--yes` (exit 6) — asserted in the round-trip gate.
- [x] ISC-352: `import --into <pack-dir> [--diff]` — `foldImportInto` (core importer) re-runs the importer with the existing pack's identity, regenerates atoms/permissions/security, preserves `metadata`/`compatibility`/`profiles`/`exports`/`adapters` verbatim, and removes stale files confined to `atoms/`. `--diff` is a zero-write preview (exit 0 = in sync, 2 = out of sync, tree-snapshot-asserted); apply prints per-file adds/changes/removals and defers consent to the user's git commit. `--id`/`--out` conflict with `--into` (usage error).
- [x] ISC-353: e2e gate (CI-runnable, no network) in `update.cli.test.ts` on the S1 mock-GitHub harness with throwaway `HOME`s: import a fixture `~/.claude` (skill + hook with bundled script) → serve the pack → `install --scope user` onto a second HOME → `verify --all` clean → instruction-only pack edit updates under `--yes` alone → hook-script edit refuses without `--allow-exec`, applies with it, verify clean again. HOME tree diff across `install --dry-run` AND `update --dry-run` asserted **empty**; `install --scope user --dry-run` with no `~/.claude` errors without creating it.
...
## Lockfile v2 — multi-pack AGENTPACK.lock (2026-07-17, #114)

The `docs/sync-design.md` §6 risk item: `AGENTPACK.lock` was single-pack and `applyInstall` replaced it wholesale, so a multi-pack project silently kept only the last-installed pack's lock. `lockfileVersion: 2` holds every installed pack.

### Lockfile-v2 ISCs

- [x] ISC-358: v2 document shape — `{ lockfileVersion: 2, packs: { [packId]: entry } }` where each entry mirrors the v1 fields exactly (`packId`, `packVersion`, `target`, `profile`, `generator`, `manifestChecksum`, `canonicalization`, `atoms`, `dependencies`, `signatures`, optional `source`). Schema (`lockfileV2Schema`) rejects a `packs` key that disagrees with its entry's `packId`. Serialization keeps the canonical sorted-key, 2-indent discipline — determinism holds regardless of install order (`lockfile-v2.test.ts`).
- [x] ISC-359: migration read path — `parseLockfileDocument` reads v1 OR v2; a v1 file is interpreted as a single-pack v2 in memory everywhere (install-over, verify, update, uninstall) and the first write persists v2. Unknown versions are refused loudly. An entry renders back to a byte-identical standalone v1 document (`lockfileEntryAsV1` → `serializeLockfile`), so the per-pack checksum (`lockfileEntryChecksum`) equals the whole-file hash pre-v2 CLIs recorded in install manifests — old manifests stay valid with zero rewrites (`lockfile-v2.test.ts`, `lockfile-v2-multipack.test.ts`).
- [x] ISC-360: `applyInstall` merges instead of replacing — the prior lockfile is parsed BEFORE the WAL begin entry (a corrupt/unrecognized lockfile refuses the install with zero writes rather than silently dropping other packs' entries), the pack's entry is upserted under the project lock, prior bytes are backed up as before. Installing pack B preserves pack A's entry; re-installing/updating A replaces only A's entry.
- [x] ISC-361: uninstall removes only its own entry — read-modify-write under the project lock (same serialization as install); when the last entry goes the FILE is deleted (decision below). New `UninstallResult.lockfile` outcome: `entry-removed | file-removed | not-tracked | unrecognized-left-in-place`; a corrupt lockfile never bricks uninstall.
- [x] ISC-362: reader sweep — every lockfile consumer moved to the document parser: core `verifyInstall` cross-checks the per-pack entry checksum (no longer whole-file bytes, so another pack's install can't shift it); CLI `verify --sig` reads the signature + `manifestChecksum` from THIS pack's entry (pre-v2 it read whichever pack installed last) and fails loudly (`lockfile_missing_pack`) when the entry is absent; `scripts/smoke-e2e.sh` jq path updated. `update` already keyed on install manifests and inherits the merge via `applyInstall`.

### Lockfile-v2 decisions

- **Uninstall deletes the file with the last entry** — the lockfile describes the _currently installed set_ (reproducibility artifact: clone + install from lock); `.agentpack/history.jsonl` is the audit trail. This supersedes the Phase-2 "lockfile retained on uninstall for audit" posture.
...
- [x] ISC-366: fold preservation — `foldImportInto` gains `sourceTarget`; another runtime's variants (manifest entries AND files) survive an `import --into` fold, while the fold source's own variant is superseded by the fresh content. An atom deleted from the fold source is carried forward as a **variant-only atom** when foreign variants remain (identity + preserved variants, no default — the source runtime owned and deleted its own content); when only the source target's variant existed, the atom and its files are genuinely gone. CLI maps `--from` → source target (`atom-variants-fold.test.ts`).
...
- [x] ISC-368 (review round): `pack chat` / `pack mcpb` do NOT resolve variants (they bypass the planner's `selectAtomVariants`) — the limitation is now loud instead of silent: a variant-only atom compiles to a description fallback (skills) or is skipped (`connectors.json`) with an explicit warning naming the true reason (no more misleading ``Skill directory `undefined` has no SKILL.md``); `.mcpb` bundling warns when a bundled server declares variants; the CLI prints both exporters' warnings (`atom-variants-export.test.ts`).
...
## Sync — user-scope Codex and Antigravity round trips (2026-07-17, #132)

User scope was Claude Code-only; Codex and generic targets were hard-rejected, so AgentPack could not carry a shared Claude/Codex/Antigravity workstation setup. `--scope user` now supports three runtimes, each single-rooted at its own user config dir.

### User-scope multi-runtime ISCs

- [x] ISC-369: user-scope root + mapping dispatch — `install/userScope.ts` (`USER_SCOPE_TARGETS`, `userScopeRoot`, `allUserScopeRoots`, `mapOutputToUserScope`) maps `claude-code` → `~/.claude`, `codex` → `~/.codex`, `generic` → `~/.gemini/config`; `planInstall`'s scope remap dispatches per target and every other target refuses `--scope user` loudly (planner AND CLI usage error, pinned by test). The old claude-only rejection pins were replaced by new-boundary pins (`cursor` still refuses).
- [x] ISC-370: codex user layout — `mapCodexOutputToUserScope` remaps `AGENTS.md` → `AGENTS.md`, `.codex/{config.toml,hooks.json,agents/*}` → root-level twins, `.agents/skills/…` → `skills/…` (the codex adapter receives the install `scope` and renders the generated `AGENTS.md` skill index for that layout; authored text is never rewritten — #193) — the same home-style layout `import --from codex ~/.codex` reads back, so install→import round-trips on identical paths (`user-scope-mapping.test.ts`).
- [x] ISC-371: TOML config merge — `.codex/config.toml` (and its mapped `config.toml` form) joins the deep-merge discipline: new `"toml"` merge strategy (`mergeTomlConfig`/`forceMergeTomlConfig`/`removeTomlFragment`/`tomlFragmentIntact`) shares the object-level merge semantics with the JSON path (prior-fragment replacement, collision refusal, `--force` wins ONLY collided keys, dunder-key refusal); verify checks the fragment, uninstall/update-removals surgically un-merge. Trust settings, credentials pointers, and unrelated machine entries survive install/update/uninstall (`merge-toml.test.ts`, `user-scope-codex-install.test.ts`). Honest ceiling: serialization is canonical, so comments in an existing `config.toml` are not preserved (warned at plan time).
- [x] ISC-372: Antigravity via generic (decision, not adapter) — `install --target generic --scope user` roots at `~/.gemini/config`, Antigravity's global customization root ("Global Discovery"), which reads standalone `AGENTS.md` rule files and Agent-Skills `skills/` folders directly in that directory — verified two-floor against agy 1.1.3 (embedded customization docs in the binary + the live machine layout). The mapping is the identity; the user's own `GEMINI.md`, `mcp_config.json`, and hand-placed skills are untouched (`user-scope-generic.cli.test.ts`).
- [x] ISC-373: multi-root update/uninstall — `update --scope user` (check AND apply) scans every existing user root for `.agentpack/` state and reconciles each with the scope its install manifest recorded; `uninstall <packId> --scope user` removes the pack from every root that has it; roots without state are skipped silently, and a packId found in no root still exits 8.
- [x] ISC-374: exec gates at codex user scope — `computeExecDelta` recognizes the mapped codex exec surfaces (`hooks.json`, `config.toml`, keyed off the manifest's recorded user scope); a hook-command change on update refuses without a fresh `--allow-exec` even with `--yes` (exit 6), and `--force` never implies `--allow-exec` at install (exit 6, zero writes) — both pinned per-runtime in the e2e gates.
- [x] ISC-375: two-throwaway-HOME e2e gates (CI-runnable, no network) — codex: seed a live `~/.codex` (AGENTS.md, skill, hooks.json hook, config.toml MCP server) → `import --from codex` → serve via mock GitHub → `install --scope user --target codex` onto a second HOME seeded with machine-local `config.toml` (trust settings survive) → verify clean → instruction update under `--yes` → hook-command update exec-refused then applied → uninstall leaves machine config intact. Generic: same loop against `~/.gemini/config` seeded with live `GEMINI.md`/`mcp_config.json`/own skills. sha256 tree snapshots pin `install --dry-run` AND `update --dry-run` byte-identical (`user-scope-codex.cli.test.ts`, `user-scope-generic.cli.test.ts`).
- [x] ISC-376 (found during #132): update no longer drops merge records for unchanged merged files — `applyUpdate`'s filtered plan now carries merge records for `unchanged` paths, so a shared `settings.json`/`config.toml` holding user content stays fragment-tracked across updates instead of degrading to whole-file tracking (which read as false drift on the next verify and would have cost uninstall its surgical un-merge).

### User-scope multi-runtime decisions
