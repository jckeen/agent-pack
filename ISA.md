...
- [x] ISC-370: codex user layout — `mapCodexOutputToUserScope` remaps `AGENTS.md` → `AGENTS.md`, `.codex/{config.toml,hooks.json,agents/*}` → root-level twins, `.agents/skills/…` → `skills/…` (with the `AGENTS.md` skill index rewritten to match) — the same home-style layout `import --from codex ~/.codex` reads back, so install→import round-trips on identical paths (`user-scope-mapping.test.ts`).
