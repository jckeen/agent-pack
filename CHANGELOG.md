...
- **Review hardening (Codex refuter + Claude security review on PR #121, all fixed failing-test-first).** (a) `import --into` no longer swallows failed stale-atom deletions — each failure is reported per-file and the command exits 1 instead of claiming a clean fold (#122). (b) HIGH: `--force` no longer bypasses the exec consent scan — forced conflict files join the bang-bash scan, so a pre-existing user file at a `command`/`subagent` path can't turn `--force -y` into silent exec consent (both scopes; pre-existing on master, surfaced by user scope). (c) MEDIUM: a forced JSON-collision write is now the deep-merge with the pack winning only the collided keys — never the bare fragment that replaced the user's whole `settings.json`/`.mcp.json`; the forced merge is recorded as a json merge so `verify` stays clean and `uninstall` stays surgical, and the same staged content flows to `update --theirs`. No-`--force` refusals are unchanged.
...
- **Three-way reconcile (BASE/LOCAL/NEW).** `planUpdate` compares each file's on-disk state and the new version against what the pack wrote at install (manifest hashes; merge _fragments_ for `CLAUDE.md`/`AGENTS.md`/JSON — user content around the span never conflicts). Clean updates apply; a local edit whose upstream didn't move is **retained** and reported; both-changed is a **conflict** that refuses (exit 2) until resolved per-glob with `--theirs` (local edit backed up, restorable) or `--keep-local`. Markerless pack-owned files (skills, `agentpack.json`) update cleanly via manifest-hash ownership — previously any upstream change to them would have been an install-level conflict.
- **Surgical removals.** Files whose atoms were deleted upstream are removed (fragment unmerge / owned-file delete / pre-install-backup restore); user-edited removal targets are skipped and reported, never deleted.
- **Install-grade gates on the delta.** Unsigned exec-bearing updates (added hook/`mcp_server` atoms, or writes touching hook scripts / MCP configs / bang-bash bodies) refuse without `--allow-exec` even with `--yes`. New policy `update` section: `channel` ceiling (enforced against the **live-derived** channel — a tampered manifest can't fake pinned→branch, per the #111 security note), `requireReconsent` (exec|always|never; the unsigned exec floor never lowers), `maxRiskEscalation` (installs now record their risk level as the baseline).
- **WAL crash safety.** `update_begin`/`update_commit` history actions; removal targets join `requiredBackups`; the recovery sweep rolls interrupted updates back (or forward when fully written). Hardened roll-forward: the manifest's `packVersion` must match the begin row — an update crashing between file writes and the manifest write previously would have falsely rolled forward on the stale same-packId manifest.
- **CLI.** Bare `agentpack update [packId]` applies (git sources; `--to <ref>` moves pinned/tag installs and re-pins), `--dry-run` prints the full reconcile report with zero writes, manifest records `updatedAt`/`previousPackVersion`. Registry-sourced applies stay deferred until the registry live-smoke (the `--check` path fully works; `update` prints the exact signed `install` command).
- **Review hardening (parallel security + correctness passes, all fixed failing-test-first).** Security: a tampered install manifest's `backupPath` could `..`-traverse and read an arbitrary file into the project on a restore (exfiltration primitive) — the schema now confines every backup under `.agentpack/backups/` and the restore read is realpath-contained; `.codex/config.toml` (codex MCP command lines) added to the exec-surface set; the crash-recovery sweep now refuses a broken hash chain, so a forged/committed `history.jsonl` can't drive its unlink machinery. Correctness: a write-/restore-kind removal that crashed mid-apply was silently corrupted and reported as a clean rollback — removal targets are now force-restored from their backups (both in the recovery sweep and the synchronous-failure catch), and a `result:"failed"` audit row no longer marks its begin entry as resolved (which had hidden crashed applies from every future sweep).
- Gate: all four design scenarios + crash-recovery/rollback kill tests CI-green (`update-apply.cli.test.ts`, `update-engine.test.ts`); docs (`cli.md`, `policy.md`, `sync-design.md` S2 shipped, website copy), ISA ISC-342..348.
