...
**`--scope user` (sync S3; multi-runtime in [#132](https://github.com/jckeen/agent-pack/issues/132)).** Targets every user-root install instead of a project: each existing user root (`~/.claude`, `~/.codex`, `~/.gemini/config`) is scanned for `.agentpack/` state, and each pack is re-planned with the scope its install manifest recorded, so user-layout paths reconcile correctly. Roots without AgentPack state are skipped silently. Mutually exclusive with `--project`. All gates (exec re-consent, policy, conflicts, `--dry-run` zero-writes) behave identically.
...
**`--scope user` (sync S3, [#112](https://github.com/jckeen/agent-pack/issues/112); Codex + Antigravity in [#132](https://github.com/jckeen/agent-pack/issues/132)).** Installs into the **target's user config directory** instead of a project (mutually exclusive with `--project`; supported for `claude-code`, `codex`, and `generic` — other targets are a usage error). The adapter's project-layout paths are remapped to the runtime's user layout, and install state lands at `<root>/.agentpack/` (never inside a project):

| Target        | User root          | Remap                                                                                                                                                                                                                                                                                                                  |
| ------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `claude-code` | `~/.claude`        | `CLAUDE.md` → `~/.claude/CLAUDE.md`, `.claude/X` → `~/.claude/X`, `settings.json` deep-merged, hook commands rewritten `$CLAUDE_PROJECT_DIR/.claude/…` → `$HOME/.claude/…`                                                                                                                                             |
| `codex`       | `~/.codex`         | `AGENTS.md` → `~/.codex/AGENTS.md`, `.codex/config.toml` → `~/.codex/config.toml` (**deep-merged** with your live config — trust settings and unrelated entries survive), `.codex/hooks.json` → `~/.codex/hooks.json` (deep-merged), `.codex/agents/…` → `~/.codex/agents/…`, `.agents/skills/…` → `~/.codex/skills/…` |
| `generic`     | `~/.gemini/config` | No remap — Antigravity's global customization root reads standalone `AGENTS.md` and `skills/` exactly as the generic adapter emits them (verified against agy 1.1.3)                                                                                                                                                   |

Honest ceilings: for claude-code, `.mcp.json` is written under `~/.claude` for reference, but Claude Code reads **user-scope** MCP servers from `~/.claude.json`, which AgentPack never edits — register those yourself. For codex, the `config.toml` merge is data-preserving but canonical — comments in your existing file are not preserved. Every gate behaves identically (`--allow-exec`, `--allow-critical`, conflicts, backups); a `--dry-run` performs **zero writes** under `$HOME` — it won't even create a missing user root. Verify with `agentpack verify --all --project <root>`; update with `agentpack update --scope user`. Full workflow + per-runtime mapping tables: [`sync.md`](./sync.md).
...
**`--scope user` ([#146](https://github.com/jckeen/agent-pack/issues/146); multi-runtime in [#132](https://github.com/jckeen/agent-pack/issues/132)).** The exit door for `install --scope user`: scans the user roots (`~/.claude`, `~/.codex`, `~/.gemini/config`) and uninstalls the pack from every root that has it, with the same mapping install and update use. Merged configs (`~/.claude/settings.json`, `~/.codex/config.toml`) are surgically un-merged — only the pack's entries come out. Every selected root is scanned **before any root is touched**: a user-edited file or an unparsable merged config in one root refuses the whole uninstall with all roots unchanged, and a single confirmation covers the complete multi-root plan ([#194](https://github.com/jckeen/agent-pack/issues/194)). Mutually exclusive with `--project`. Audit artifacts (`<root>/.agentpack/` history/backups and `<root>/AGENTPACK.lock`) are deliberately retained, as in project scope.
...
There is no dedicated Antigravity target: Google Antigravity consumes the `generic` target's `AGENTS.md` (it auto-loads a workspace's `AGENTS.md` and `GEMINI.md` — verified against agy 1.1.0) and its skills use the same Agent Skills `SKILL.md` format. At user scope, `install --target generic --scope user` writes into `~/.gemini/config/` — Antigravity's global customization root ("Global Discovery", verified against agy 1.1.3), which reads standalone `AGENTS.md` rule files and `skills/` folders directly in that directory ([#132](https://github.com/jckeen/agent-pack/issues/132)).
