...
**`--scope user` ([#146](https://github.com/jckeen/agent-pack/issues/146); multi-runtime in [#132](https://github.com/jckeen/agent-pack/issues/132)).** The exit door for `install --scope user`: scans the user roots (`~/.claude`, `~/.codex`, `~/.gemini/config`) and uninstalls the pack from every root that has it, with the same mapping install and update use. Merged configs (`~/.claude/settings.json`, `~/.codex/config.toml`) are surgically un-merged — only the pack's entries come out. Mutually exclusive with `--project`. Audit artifacts (`<root>/.agentpack/` history/backups and `<root>/AGENTPACK.lock`) are deliberately retained, as in project scope.
