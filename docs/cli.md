...
| `6`  | policy violation (`agentpack.policy.json`), critical-risk plan without `--allow-critical`, **or** unverified plan shipping executable content (exec atoms, or a `command`/`subagent` body with a bang-bash directive) without `--allow-exec` |
