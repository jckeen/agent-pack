...
| --------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| **Claude Code — terminal / IDE**                                                  | Bidirectional: full export + `import --from claude-code`          | `.claude/` + `.mcp.json` + hooks                                    | shipped — whole-dir `import --from claude-code` ([#88](https://github.com/jckeen/agent-pack/issues/88))                     |
...
| **CoWork**                                                                        | Works: `pack plugin` + `pack mcpb`                                | Claude Code plugin (`pack plugin`) + `.mcpb`                        | shipped — `.mcpb` emitter + hooks ceiling + org-plugins positioning ([#38](https://github.com/jckeen/agent-pack/issues/38)) |
| **Claude Chat (claude.ai)**                                                       | Works: `pack chat`                                                | skill ZIPs + connector recipe + project instructions                | shipped — `pack chat` ([#40](https://github.com/jckeen/agent-pack/issues/40))                                               |
| **Codex (OpenAI)**                                                                | Bidirectional: export adapter + `import --from codex`             | `.codex/*` + AGENTS.md                                              | shipped — `import --from codex` ([#39](https://github.com/jckeen/agent-pack/issues/39))                                     |
| **ChatGPT (consumer GPT)**                                                        | Works (human-seeded): `import --from chatgpt-gpt`                 | via `pack chat`                                                     | shipped — `import --from chatgpt-gpt` + OpenAPI→MCP transpiler ([#41](https://github.com/jckeen/agent-pack/issues/41))      |
| **Agent Plugins 1.0 clients** (VS Code, GitHub Copilot, Cursor, ChatGPT, Kiro, …) | Bidirectional: `pack agent-plugin` + `import --from agent-plugin` | `plugin.json` + `skills/` + `mcp.json` + `dev.agentpack/` namespace | shipped — Agent Plugins 1.0 interop (spec: [agent-plugins.org](https://agent-plugins.org))                                  |
...
## Build sequence (shipped in iteration-10)

Built in ROI-per-effort order (rationale in each issue); all four landed in
iteration-10 — see CHANGELOG 0.7.0-dev:
