...
- `scripts/` — `bring-up-prod.sh` (operator-only, provisions Neon + R2 + GitHub OAuth), `check-doc-truth.sh` (CI doc-contract checker), `smoke-e2e.sh` (requires a deployed registry + `SMOKE_PUBLISH_TOKEN`), `seed-import.ts`.
