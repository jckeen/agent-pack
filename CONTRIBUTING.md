...
- `scripts/` — `bring-up-prod.sh` (operator-only, provisions Neon + R2 + GitHub OAuth), `smoke-e2e.sh` (requires a deployed registry + `SMOKE_PUBLISH_TOKEN`), `seed-import.ts`, `check-doc-truth.sh` (asserts `.doc-contract`; run in CI).
