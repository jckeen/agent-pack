 ...
 *
 * Paths only — content is never rewritten (#193). The AGENTS.md skill index
 * is rendered against the mapped `skills/…` location at build time (the
 * adapter receives `scope`), so authored instruction text that mentions
 * `.agents/skills/` on purpose keeps its meaning and re-imports intact.
 */
...
export const codexAdapter = defineAdapter({
  ...
  async build(options: AdapterExportOptions) {
    ...
    // The index must point at the directory THIS install writes: the
    // user-scope remap moves `.agents/skills/…` to `skills/…` by path only,
    // so the reference is rendered for that layout here (#193) rather than
    // regex-rewritten over the whole file (which also mangled authored text).
    const skillsIndexDir = options.scope === "user" ? "skills" : ".agents/skills";
    if (skillAtomsAll.length + commandAtomsAll.length > 0) {
      sections.push(`## Skills\n`);
      sections.push(
        `The following reusable procedures ship with this pack under \`${skillsIndexDir}/\`. Read the referenced SKILL.md when a task matches.\n`,
      );
      for (const atom of [...skillAtomsAll, ...commandAtomsAll]) {
        const slug = emittedSkillSlug.get(atom.id) ?? normalizeSkillSlug(slugFor(atom));
        sections.push(
          `- **${atom.name}** (\`${skillsIndexDir}/${slug}/SKILL.md\`) — ${atom.description}`,
        );
      }
      sections.push("");
    }
    ...
    // ---------- .codex/config.toml ----------
    // Provenance is keyed by pack id — `[agentpack."<pack_id>"]` — so packs
    // coexist in one config.toml (#192): the TOML merge treats `agentpack`
    // like `mcp_servers`, a map whose per-pack entries never collide unless
    // the same pack is installed twice. Pack ids always contain a dot, so a
    // quoted pack-id key can never clash with a bare singleton key.
    const meta = {
    ...
    const tomlBlocks: string[] = [
      ...
      `# Profile: ${options.profile}`,
      "",
      stringifyToml({ agentpack: { [manifest.metadata.id]: meta } }).trimEnd(),
    ];
