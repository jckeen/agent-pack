...
describe("applyInstall merges into a multi-pack lockfile", () => {
  ...
  it("aborts before any write when the lockfile turns corrupt between preflight and lock acquisition", async () => {
    const project = await tempDir("agentpack-lockv2-proj-");
    const ws = await resolveAgentpackPaths(project);
    const packDir = await tempDir("agentpack-lockv2-window-");
    await writePack(packDir, { ...A, instructionBody: "# House\n" });
    const plan = await planInstall({
      source: packDir,
      target: "claude-code",
    ...
    // Hold the project lock so the install is parked between its preflight
    // read (which sees no lockfile) and the authoritative locked read.
    await fs.mkdir(ws.historyLockFile, { recursive: true });
    await fs.writeFile(path.join(ws.historyLockFile, "nonce"), "test-holder", "utf8");

    const pending = applyInstall({ plan, actor: { type: "cli" } });
    // Sink the eventual rejection so it is never counted as unhandled; the
    // real assertion happens below.
    pending.catch(() => {});
    // Let the install pass preflight and start polling the held lock.
    await new Promise((r) => setTimeout(r, 200));
    // Corrupt the lockfile inside the window, then release the lock.
    const lockPath = path.join(project, "AGENTPACK.lock");
    await fs.writeFile(lockPath, "{ corrupted in the window", "utf8");
    await fs.rm(ws.historyLockFile, { recursive: true, force: true });

    await expect(pending).rejects.toThrow(/AGENTPACK\.lock could not be read/);
    // The authoritative locked read must abort BEFORE `.agentpack/installed`,
    // `.agentpack/backups`, or the WAL begin entry are created: nothing may
    // exist besides the corrupt lockfile the test planted.
    expect(await fs.readFile(lockPath, "utf8")).toBe("{ corrupted in the window");
    expect(
      await fs.readFile(path.join(project, "CLAUDE.md"), "utf8").catch(() => null),
    ).toBeNull();
    expect(await fs.readdir(ws.agentpackDir)).toEqual([]);
  });
