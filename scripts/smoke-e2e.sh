...
#   3. `agentpack publish examples/pr-quality` round-trips through:
...
#   4. `agentpack install agentpack-smoke/pr-quality@<v> --registry $REGISTRY_URL`
#      from a temporary clean projectRoot exits 0, writes files, writes lockfile.
#   5. `agentpack verify` against the install reports `clean: true`.
...
#   SMOKE_PUBLISH_TOKEN   a Bearer token with scope 'publish:packs:agentpack-smoke'
...
AGENTPACK_DIST="$REPO_ROOT/packages/cli/dist/index.js"
test -f "$AGENTPACK_DIST" || { red "CLI build not found at $AGENTPACK_DIST"; exit 1; }
agentpack() { node "$AGENTPACK_DIST" "$@"; }
green "  ✓ CLI built"
...
PUBLISH_OUT=$(AGENTPACK_TOKEN="$SMOKE_PUBLISH_TOKEN" \
  agentpack publish "$SMOKE_MANIFEST" \
  --registry "$REGISTRY_URL" \
  --publisher agentpack-smoke \
...
green "  ✓ published agentpack-smoke/pr-quality@$PUBLISHED_VERSION"

note "▸ Step 4/6: install into clean projectRoot"
INSTALL_ROOT="$TMP_DIR/install-target"
mkdir -p "$INSTALL_ROOT"
agentpack install "agentpack-smoke/pr-quality@$PUBLISHED_VERSION" \
  --registry "$REGISTRY_URL" \
  --target claude-code \
  --profile safe \
...
note "▸ Step 6/6: agentpack verify (drift check)"
cd "$INSTALL_ROOT"
agentpack verify agentpack-smoke.pr-quality || { red "verify reported drift"; exit 6; }
cd "$REPO_ROOT"
green "  ✓ verify clean"

# Record results for regression tracking.
ARTIFACT_BYTES=$(find "$INSTALL_ROOT" -type f -not -path '*/\.agentpack/*' -not -name 'AGENTPACK.lock' \
...
green "    version : agentpack-smoke/pr-quality@$PUBLISHED_VERSION"
